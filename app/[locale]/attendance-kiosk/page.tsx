          ...
          <aside className="border-b border-slate-200 bg-white p-5 text-slate-950 lg:border-b-0 lg:border-r">
            ...
            <div className="mt-3 max-h-[52vh] space-y-2 overflow-y-auto pr-1 lg:max-h-[calc(100vh-250px)]">
              {filtered.map((employee) => (
                <button key={employee.id} onClick={() => { setSelected(employee); setSuccess(""); setError(""); }} className={`w-full border p-4 text-left ${selected?.id === employee.id ? "border-blue-700 bg-blue-50" : "border-slate-200 bg-white hover:border-slate-400"}`}>
              ...
              {selected ? (
                <div>
                  ...
                  <div className="mt-8 grid gap-4 sm:grid-cols-2">
                    <button disabled={Boolean(posting)} onClick={() => punch("CHECK_IN")} className="flex min-h-32 flex-col items-center justify-center bg-emerald-600 p-6 text-center text-white hover:bg-emerald-700 disabled:opacity-60">
                    ...
                    <button disabled={Boolean(posting)} onClick={() => punch("CHECK_OUT")} className="flex min-h-32 flex-col items-center justify-center bg-slate-900 p-6 text-center text-white hover:bg-slate-800 disabled:opacity-60">
                  ...
                  <button onClick={() => setSelected(null)} className="mt-5 h-12 w-full border border-slate-300 bg-white text-sm font-black text-slate-700 hover:bg-slate-50">Changer d’employé</button>
                </div>
              ) : (
